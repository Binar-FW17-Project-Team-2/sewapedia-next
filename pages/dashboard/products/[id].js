...
function FormEdit() {
  ...
  async function getProduct() {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_BASE_URL}/api/v1/product/${id}`
    )
  ...
  async function deleteImgFromFirebase() {
    deletedImage.forEach(async (value) => {
  ...
  function deleteImage(idx) {
    setDeletedImage((prev) => [...prev, imageUrl[idx]])
